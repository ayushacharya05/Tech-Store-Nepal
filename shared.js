...
async function ensureSeeded() {
    try {
        ...
        const { data: esewa } = await db.from('meta').select('list').eq('id', 'esewa').single();
        if (!esewa) {
            await db.from('meta').upsert({ id: 'esewa', list: SEED_ESEWA }, { onConflict: 'id' });
        }
    } catch (e) {
...
function subscribeEsewaConfig(callback) {
    const fetchAndCallback = async () => {
        const { data, error } = await db.from('meta').select('list').eq('id', 'esewa').single();
        if (!error && data) callback(data.list || SEED_ESEWA);
        else callback(SEED_ESEWA);
    };

    fetchAndCallback();

    const channel = db.channel('realtime:esewa')
        .on('postgres_changes', { event: '*', schema: 'public', table: 'meta', filter: 'id=eq.esewa' }, () => {
            fetchAndCallback();
        })
        .subscribe();
...
async function saveEsewaConfig(config) {
    const { error } = await db.from('meta').upsert({ id: 'esewa', list: config }, { onConflict: 'id' });
